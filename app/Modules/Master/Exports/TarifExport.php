...
class TarifExport implements FromCollection, WithHeadings, WithMapping
{
    ...
    public function map($item): array { return [$item->kode_tindakan, $item->kode_asuransi, number_format($item->tarif, 0, ',', '.'), number_format($item->jasmed, 0, ',', '.'), number_format($item->bhp, 0, ',', '.'), $item->status]; }
    public function headings(): array { return ['Kode Tindakan', 'Kode Asuransi', 'Tarif', 'Jasmed', 'BHP', 'Status']; }
}
