...
Route::middleware(['web', 'auth'])->prefix('admisi/antrian')->name('antrian.')->group(function () {
    Route::get('/', AntrianPage::class)->name('index');
    Route::get('/ambil', AmbilAntrianPage::class)->name('ambil');
    Route::get('/cetak/{id}', \App\Modules\Antrian\Http\Livewire\CetakAntrianPage::class)->name('cetak')->middleware('signed');
});
