        <div x-data="{ showModal: false, init(){this.$watch('showModal',v=>{if(v){$nextTick(()=>{this.$refs.firstInput&&this.$refs.firstInput.focus()})}})} }" @open-modal.window="showModal=true" @close-modal.window="showModal=false" x-init="init()">
            ...
            <div class="page-header">
                <div class="page-header-title">
                    ...
                    <div>
                        <h1 class="text-xl font-bold tracking-tight text-[#2c3e50]">Master Data Asuransi</h1>
                        <p class="text-xs text-[#878a99] font-medium mt-0.5">Kelola data asuransi dan penjamin pembayaran.</p>
                    </div>
                </div>
                <div class="page-header-breadcrumb">
                    ...
                    <span class="sep text-gray-300">/</span>
                    <span class="text-[#405189] font-bold">Asuransi</span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 mb-8">
                <div class="group relative overflow-hidden bg-white rounded-2xl p-5 shadow-sm hover:shadow-xl transition-all duration-500 border-l-4 border-[#405189]">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-[#878a99] font-bold text-[10px] uppercase tracking-[0.1em]">Total Asuransi</p>
                            <h4 class="text-2xl font-black text-[#2c3e50] leading-none mt-1">{{ number_format($totalAsuransi) }}</h4>
                        </div>
                    </div>
                </div>
                <div class="group relative overflow-hidden bg-white rounded-2xl p-5 shadow-sm hover:shadow-xl transition-all duration-500 border-l-4 border-[#0ab39c]">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-[#878a99] font-bold text-[10px] uppercase tracking-[0.1em]">Asuransi Aktif</p>
                            <h4 class="text-2xl font-black text-[#2c3e50] leading-none mt-1 text-[#0ab39c]">{{ number_format($asuransiAktif) }}</h4>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex flex-wrap items-center gap-4 w-full lg:w-auto">
                        <div class="relative flex-grow min-w-[280px]">
                            ...
                            <input type="text" wire:model.live.debounce.300ms="search" 
                                   ...
                                   placeholder="Cari kode atau nama asuransi...">
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3 w-full lg:flex lg:w-auto lg:items-center lg:gap-1.5 lg:p-1 lg:rounded-lg lg:border lg:border-[#e2e8f0] lg:bg-white">
                            <a href="{{ route('master.asuransi.print', ['status' => $selectedStatus]) }}" target="_blank" 
                            ...
                            <a href="{{ route('master.asuransi.export', ['status' => $selectedStatus]) }}" target="_blank" 
                        ...
                        <button @click="$wire.create()" class="btn btn-primary h-10 px-6 shadow-sm flex items-center justify-center gap-2 transition-all hover:translate-y-[-2px] hover:shadow-lg active:scale-95 w-full lg:w-auto">
                            ...
                            <span class="font-bold text-xs uppercase tracking-wider">Tambah Asuransi</span>
                        </button>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50/50">
                                ...
                                <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50">Nama Asuransi</th>
                                <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50">Tipe</th>
                                <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50">Diskon</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @forelse($this->asuransis as $item)
                            <tr wire:key="asuransi-{{ $item->id }}" class="asuransi-row transition-all duration-200">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="asuransi-code-chip shadow-sm">{{ $item->kode_asuransi }}</span>
                                </td>
                                <td class="px-6 py-4 min-w-[250px]">
                                    <div class="group relative">
                                        <div class="font-bold text-[#2c3e50] text-sm group-hover:text-[#405189] transition-colors line-clamp-1">{{ $item->nama_asuransi }}</div>
                                        <div class="text-[11px] text-gray-400 font-medium italic mt-1 leading-relaxed line-clamp-1 group-hover:line-clamp-none transition-all duration-300">
                                            {{ $item->email ?: 'Email belum terdaftar.' }}
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @php
                                        $tipColor = match($item->tipe_asuransi) {
                                            'Pemerintah' => 'bg-blue-50 text-blue-600',
                                            'Swasta' => 'bg-indigo-50 text-indigo-600',
                                            default => 'bg-gray-50 text-gray-500'
                                        };
                                    @endphp
                                    <span class="tipe-pill {{ $tipColor }}">
                                        {{ $item->tipe_asuransi }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="font-bold text-[#2c3e50]">{{ number_format($item->diskon, 1) }}%</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm text-gray-600">{{ $item->no_telepon ?: '-' }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <button @click="if('{{ $item->status }}'==='Tidak Aktif'){Swal.fire({title:'Informasi',text:'Data yang tidak aktif tidak dapat dihapus lagi.',icon:'info',confirmButtonColor:'#405189'})}else{Swal.fire({title:'Konfirmasi Nonaktif',text:'Apakah Anda yakin ingin menonaktifkan asuransi {{ $item->nama_asuransi }}?',icon:'warning',showCancelButton:true,confirmButtonColor:'#f06548',cancelButtonColor:'#6c757d',confirmButtonText:'Ya, Nonaktifkan',cancelButtonText:'Batal',reverseButtons:true}).then((r)=>{if(r.isConfirmed){$wire.delete({{ $item->id }})}})}" 
                ...
                <div class="px-6 py-5 sm:px-8 sm:py-6 bg-gray-50/50 border-t border-gray-100 pagination-custom">
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-5">
                        <div class="text-[11px] font-bold text-[#878a99] tracking-tight text-center sm:text-left">
                            ...
                            <span class="text-[#405189] font-black">{{ $this->asuransis->firstItem() }} - {{ $this->asuransis->lastItem() }}</span> 
                            dari <span class="text-[#405189] font-black">{{ number_format($this->asuransis->total()) }}</span> 
                            <span class="hidden sm:inline">asuransi terdaftar</span>
                            <span class="sm:hidden">total data</span>
                        </div>
                        {{ $this->asuransis->links() }}
                    </div>
                </div>
                @endif
            </div>
                    ...
                    <div class="px-5 py-4 sm:px-8 sm:py-6 bg-gradient-to-r from-gray-50 to-white border-b border-gray-100 flex items-center justify-between">
                        <div class="flex items-center gap-2 sm:gap-3">
                            <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-indigo-50 text-[#405189] flex items-center justify-center shadow-inner">
                                <i class="ri-shield-check-line text-lg sm:text-xl"></i>
                            </div>
                            <div>
                                <h5 class="text-sm sm:text-base font-black text-[#2c3e50] tracking-tight">{{ $isEdit ? 'Update Data Asuransi' : 'Asuransi Baru' }}</h5>
                                <p class="text-[9px] sm:text-[10px] text-gray-400 font-bold uppercase tracking-widest hidden sm:block">Lengkapi informasi asuransi di bawah</p>
                            </div>
                        </div>
                        ...
                        <form wire:submit.prevent="save" class="space-y-5 sm:space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
                                <div class="space-y-1.5">
                                    <label class="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Kode Asuransi <span class="text-rose-500">*</span></label>
                                    <input type="text" wire:model="kode_asuransi" x-ref="firstInput" 
                                           class="w-full bg-gray-50 border border-gray-100 rounded-xl sm:rounded-2xl py-2.5 sm:py-3 px-4 text-sm font-black text-[#405189] uppercase tracking-wider focus:bg-white focus:ring-4 focus:ring-indigo-100 focus:border-[#405189] transition-all outline-none @error('kode_asuransi') border-rose-300 bg-rose-50/30 @enderror {{ ($isEdit || $kodeReadonly) ? 'bg-gray-100 cursor-not-allowed' : '' }}" 
                                           placeholder="ASR00001" {{ ($isEdit || $kodeReadonly) ? 'readonly' : '' }}>
                                    @error('kode_asuransi') <span class="text-[10px] text-rose-500 font-bold px-1">{{ $message }}</span> @enderror
                                </div>
                                <div class="space-y-1.5">
                                    <label class="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Tipe Asuransi</label>
                                    <x-custom-dropdown 
                                        model="tipe_asuransi" 
                                        :options="[
                                    ...
                                    />
                                    @error('tipe_asuransi') <span class="text-[10px] text-rose-500 font-bold px-1">{{ $message }}</span> @enderror
                                </div>
                            </div>

                            <div class="space-y-1.5">
                                <label class="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Nama Asuransi <span class="text-rose-500">*</span></label>
                                <input type="text" wire:model="nama_asuransi" 
                                       class="w-full bg-gray-50 border border-gray-100 rounded-xl sm:rounded-2xl py-2.5 sm:py-3 px-4 text-sm font-bold text-[#2c3e50] focus:bg-white focus:ring-4 focus:ring-indigo-100 focus:border-[#405189] transition-all outline-none @error('nama_asuransi') border-rose-300 bg-rose-50/30 @enderror" 
                                       placeholder="Contoh: BPJS Kesehatan">
                                @error('nama_asuransi') <span class="text-[10px] text-rose-500 font-bold px-1">{{ $message }}</span> @enderror
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
                                <div class="space-y-1.5">
                                    <label class="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Diskon (%)</label>
                                    <input type="number" step="0.01" wire:model="diskon" 
                            ...
                            <div class="space-y-1.5">
                                <label class="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Email</label>
                                <input type="email" wire:model="email" 
                                       class="w-full bg-gray-50 border border-gray-100 rounded-xl sm:rounded-2xl py-2.5 sm:py-3 px-4 text-sm font-bold text-[#2c3e50] focus:bg-white focus:ring-4 focus:ring-indigo-100 focus:border-[#405189] transition-all outline-none @error('email') border-rose-300 bg-rose-50/30 @enderror" 
                                       placeholder="email@asuransi.com">
                                @error('email') <span class="text-[10px] text-rose-500 font-bold px-1">{{ $message }}</span> @enderror
                            </div>
                            ...
                            <div class="flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-xl sm:rounded-2xl border border-dashed border-gray-200">
                                <div class="flex items-center gap-2 sm:gap-3">
                                    ...
                                    <div>
                                        <p class="text-[8px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none">Status Asuransi</p>
