        <div x-data="{ showModal: false, init(){this.$watch('showModal',v=>{if(v){$nextTick(()=>{this.$refs.firstInput&&this.$refs.firstInput.focus()})}})} }" @open-modal.window="showModal=true" @close-modal.window="showModal=false" x-init="init()">
            ...
            <div class="page-header">
                <div class="page-header-title">
                    ...
                    <div>
                        <h1 class="text-xl font-bold tracking-tight text-[#2c3e50]">Master Data Diagnosis</h1>
                        <p class="text-xs text-[#878a99] font-medium mt-0.5">Kelola data ICD-10 untuk diagnosa medis pasien.</p>
                    </div>
                </div>
                <div class="page-header-breadcrumb">
                    ...
                    <span class="sep text-gray-300">/</span>
                    <span class="text-[#405189] font-bold">Diagnosis</span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 mb-8">
                <div class="group relative overflow-hidden bg-white rounded-2xl p-5 shadow-sm hover:shadow-xl transition-all duration-500 border-l-4 border-[#405189]">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-[#878a99] font-bold text-[10px] uppercase tracking-[0.1em]">Total Diagnosis</p>
                            <h4 class="text-2xl font-black text-[#2c3e50] leading-none mt-1">{{ number_format($totalDiagnosis) }}</h4>
                        </div>
                    </div>
                </div>
                <div class="group relative overflow-hidden bg-white rounded-2xl p-5 shadow-sm hover:shadow-xl transition-all duration-500 border-l-4 border-[#0ab39c]">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-[#878a99] font-bold text-[10px] uppercase tracking-[0.1em]">Diagnosis Aktif</p>
                            <h4 class="text-2xl font-black text-[#2c3e50] leading-none mt-1 text-[#0ab39c]">{{ number_format($diagnosisAktif) }}</h4>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 border-b border-gray-50 flex flex-col lg:flex-row justify-between items-center gap-6 glass-header sticky top-0 z-20">
                    ...
                    <div class="flex flex-wrap items-center gap-4 w-full lg:w-auto">
                        ...
                        <div class="grid grid-cols-2 gap-3 w-full lg:flex lg:w-auto lg:items-center lg:gap-1.5 lg:p-1 lg:rounded-lg lg:border lg:border-[#e2e8f0] lg:bg-white">
                            <a href="{{ route('master.diagnosis.print', ['status' => $selectedStatus]) }}" target="_blank" 
                            ...
                            <a href="{{ route('master.diagnosis.export', ['status' => $selectedStatus]) }}" target="_blank" 
                        ...
                        <button @click="$wire.create()" class="btn btn-primary h-10 px-6 shadow-sm flex items-center justify-center gap-2 transition-all hover:translate-y-[-2px] hover:shadow-lg active:scale-95 w-full lg:w-auto">
                            ...
                            <span class="font-bold text-xs uppercase tracking-wider">Tambah Diagnosis</span>
                        </button>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50/50">
                                <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50">ICD-10 Code</th>
                                <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50">Diagnosis Details</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @forelse($this->diagnoses as $item)
                            <tr wire:key="diag-{{ $item->id }}" class="diagnosis-row transition-all duration-200">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="icd10-chip shadow-sm">{{ $item->kode_diagnosa }}</span>
                                </td>
                                <td class="px-6 py-4 min-w-[300px]">
                                    <div class="group relative">
                                        <div class="font-bold text-[#2c3e50] text-sm group-hover:text-[#405189] transition-colors line-clamp-1">{{ $item->nama_diagnosa }}</div>
                                        <div class="text-[11px] text-gray-400 font-medium italic mt-1 leading-relaxed line-clamp-1 group-hover:line-clamp-none transition-all duration-300">
                                            {{ $item->deskripsi ?: 'Tidak ada deskripsi tambahan untuk kode ini.' }}
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @php
                                        $catColor = match($item->kategori) {
                                            'Gigi/Bedah Mulut' => 'bg-blue-50 text-blue-600',
                                            'Penyakit Umum' => 'bg-emerald-50 text-emerald-600',
                                            'Bedah' => 'bg-rose-50 text-rose-600',
                                            default => 'bg-gray-50 text-gray-500'
                                        };
                                    @endphp
                                    <span class="category-pill {{ $catColor }}">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <button @click="if('{{ $item->status }}'==='Tidak Aktif'){Swal.fire({title:'Informasi',text:'Data yang tidak aktif tidak dapat dihapus lagi.',icon:'info',confirmButtonColor:'#405189'})}else{Swal.fire({title:'Konfirmasi Nonaktif',text:'Apakah Anda yakin ingin menonaktifkan kode {{ $item->kode_diagnosa }}?',icon:'warning',showCancelButton:true,confirmButtonColor:'#f06548',cancelButtonColor:'#6c757d',confirmButtonText:'Ya, Nonaktifkan',cancelButtonText:'Batal',reverseButtons:true}).then((r)=>{if(r.isConfirmed){$wire.delete({{ $item->id }})}})}" 
                ...
                <div class="px-6 py-5 sm:px-8 sm:py-6 bg-gray-50/50 border-t border-gray-100 pagination-custom">
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-5">
                        <div class="text-[11px] font-bold text-[#878a99] tracking-tight text-center sm:text-left">
                            ...
                            <span class="text-[#405189] font-black">{{ $this->diagnoses->firstItem() }} - {{ $this->diagnoses->lastItem() }}</span> 
                            dari <span class="text-[#405189] font-black">{{ number_format($this->diagnoses->total()) }}</span> 
                            <span class="hidden sm:inline">diagnosis terdaftar</span>
                            <span class="sm:hidden">total data</span>
                        </div>
                        {{ $this->diagnoses->links() }}
                    </div>
                </div>
                @endif
            </div>
                    ...
                    <div class="px-5 py-4 sm:px-8 sm:py-6 bg-gradient-to-r from-gray-50 to-white border-b border-gray-100 flex items-center justify-between">
                        <div class="flex items-center gap-2 sm:gap-3">
                            <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-indigo-50 text-[#405189] flex items-center justify-center shadow-inner">
                                <i class="ri-edit-2-fill text-lg sm:text-xl"></i>
                            </div>
                            <div>
                                <h5 class="text-sm sm:text-base font-black text-[#2c3e50] tracking-tight">{{ $isEdit ? 'Update Diagnosa' : 'Data Diagnosa Baru' }}</h5>
                                <p class="text-[9px] sm:text-[10px] text-gray-400 font-bold uppercase tracking-widest hidden sm:block">Lengkapi parameter ICD-10 di bawah</p>
                            </div>
                        </div>
                        ...
                        <form wire:submit.prevent="save" class="space-y-5 sm:space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
                                <div class="space-y-1.5">
                                    <label class="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Kode ICD-10 <span class="text-rose-500">*</span></label>
                                    <input type="text" wire:model="kode_diagnosa" x-ref="firstInput" 
                                           class="w-full bg-gray-50 border border-gray-100 rounded-xl sm:rounded-2xl py-2.5 sm:py-3 px-4 text-sm font-black text-[#405189] uppercase tracking-wider focus:bg-white focus:ring-4 focus:ring-indigo-100 focus:border-[#405189] transition-all outline-none @error('kode_diagnosa') border-rose-300 bg-rose-50/30 @enderror" 
                                           placeholder="K02.1">
                                    @error('kode_diagnosa') <span class="text-[10px] text-rose-500 font-bold px-1">{{ $message }}</span> @enderror
                                </div>
                                <div class="space-y-1.5">
                                    <label class="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Kategori</label>
                                    <x-custom-dropdown 
                                        model="kategori" 
                                        :options="[
                                            ['value' => 'Gigi/Bedah Mulut', 'label' => 'Dental (K02-K08)', 'icon' => 'ri-mastodon-line text-blue-500'],
                                            ['value' => 'Penyakit Umum', 'label' => 'Umum/Internal', 'icon' => 'ri-heart-pulse-line text-emerald-500'],
                                            ['value' => 'Bedah', 'label' => 'Spesialis Bedah', 'icon' => 'ri-scissors-line text-rose-500']
                                        ]"
                            ...
                            <div class="space-y-1.5">
                                <label class="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Nama Diagnosa <span class="text-rose-500">*</span></label>
                                <textarea wire:model="nama_diagnosa" rows="2" 
                                          class="w-full bg-gray-50 border border-gray-100 rounded-xl sm:rounded-2xl py-2.5 sm:py-3 px-4 text-sm font-bold text-[#2c3e50] focus:bg-white focus:ring-4 focus:ring-indigo-100 focus:border-[#405189] transition-all outline-none resize-none @error('nama_diagnosa') border-rose-300 bg-rose-50/30 @enderror" 
                                          placeholder="Contoh: Dental caries on pit and fissure surface..."></textarea>
                                @error('nama_diagnosa') <span class="text-[10px] text-rose-500 font-bold px-1">{{ $message }}</span> @enderror
                            </div>

                            <div class="space-y-1.5">
                                <label class="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Deskripsi Indonesia</label>
                                <textarea wire:model="deskripsi" rows="2" sm:rows="3" 
                                          ...
                                          placeholder="Masukkan deskripsi atau terjemahan..."></textarea>
                            </div>
                            ...
                            <div class="flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-xl sm:rounded-2xl border border-dashed border-gray-200">
                                <div class="flex items-center gap-2 sm:gap-3">
                                    ...
                                    <div>
                                        <p class="text-[8px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none">Status Data</p>
