        <div x-data="{ showModal: false, init(){this.$watch('showModal',v=>{if(v){$nextTick(()=>{this.$refs.firstInput&&this.$refs.firstInput.focus()})}})} }" @open-modal.window="showModal=true" @close-modal.window="showModal=false" x-init="init()">
            ...
            <div class="page-header">
                <div class="page-header-title">
                    ...
                    <div>
                        <h1 class="text-xl font-bold tracking-tight text-[#2c3e50]">Master Data Menu</h1>
                        <p class="text-xs text-[#878a99] font-medium mt-0.5">Kelola menu navigasi dan struktur aplikasi.</p>
                    </div>
                </div>
                <div class="page-header-breadcrumb">
                    ...
                    <span class="sep text-gray-300">/</span>
                    <span class="text-[#405189] font-bold">Menu</span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 mb-8">
                <div class="group relative overflow-hidden bg-white rounded-2xl p-5 shadow-sm hover:shadow-xl transition-all duration-500 border-l-4 border-[#405189]">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-[#878a99] font-bold text-[10px] uppercase tracking-[0.1em]">Total Menu</p>
                            <h4 class="text-2xl font-black text-[#2c3e50] leading-none mt-1">{{ number_format($totalMenu) }}</h4>
                        </div>
                    </div>
                </div>
                <div class="group relative overflow-hidden bg-white rounded-2xl p-5 shadow-sm hover:shadow-xl transition-all duration-500 border-l-4 border-[#0ab39c]">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-[#878a99] font-bold text-[10px] uppercase tracking-[0.1em]">Menu Aktif</p>
                            <h4 class="text-2xl font-black text-[#2c3e50] leading-none mt-1 text-[#0ab39c]">{{ number_format($menuAktif) }}</h4>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex flex-wrap items-center gap-4 w-full lg:w-auto">
                        <div class="relative flex-grow min-w-[280px]">
                            ...
                            <input type="text" wire:model.live.debounce.300ms="search" 
                                   ...
                                   placeholder="Cari nama atau link menu...">
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3 w-full lg:flex lg:w-auto lg:items-center lg:gap-1.5 lg:p-1 lg:rounded-lg lg:border lg:border-[#e2e8f0] lg:bg-white">
                            <a href="{{ route('master.menu.print', ['status' => $selectedStatus]) }}" target="_blank" 
                            ...
                            <a href="{{ route('master.menu.export', ['status' => $selectedStatus]) }}" target="_blank" 
                        ...
                        <button @click="$wire.create()" class="btn btn-primary h-10 px-6 shadow-sm flex items-center justify-center gap-2 transition-all hover:translate-y-[-2px] hover:shadow-lg active:scale-95 w-full lg:w-auto">
                            ...
                            <span class="font-bold text-xs uppercase tracking-wider">Tambah Menu</span>
                        </button>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50/50">
                                <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50">Nama Menu</th>
                                <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50">Link</th>
                                <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50">Icon</th>
                                <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50">Parent ID</th>
                                <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50">Urutan</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr wire:key="menu-{{ $item->id }}" class="menu-row transition-all duration-200">
                                <td class="px-6 py-4 min-w-[250px]">
                                    <div class="group relative">
                                        <div class="font-bold text-[#2c3e50] text-sm group-hover:text-[#405189] transition-colors line-clamp-1">{{ $item->menu_name }}</div>
                                        <div class="text-[11px] text-gray-400 font-medium italic mt-1 leading-relaxed line-clamp-1 group-hover:line-clamp-none transition-all duration-300">
                                            {{ $item->menu_link ?: 'Tidak ada link.' }}
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm text-gray-600">{{ $item->menu_link ?: '-' }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($item->menu_icon)
                                    <span class="icon-preview">
                                        <i class="{{ $item->menu_icon }}"></i>
                                        <span class="font-mono text-[10px]">{{ $item->menu_icon }}</span>
                                    </span>
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm text-gray-600">{{ $item->parent_id ?: '-' }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="font-bold text-[#2c3e50]">{{ $item->order_no }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($item->is_active)
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <button @click="if(!{{ $item->is_active ? 'true' : 'false' }}){Swal.fire({title:'Informasi',text:'Menu yang sudah tidak aktif tidak dapat dihapus.',icon:'info',confirmButtonColor:'#405189'})}else{Swal.fire({title:'Konfirmasi Nonaktif',text:'Apakah Anda yakin ingin menonaktifkan menu ini?',icon:'warning',showCancelButton:true,confirmButtonColor:'#f06548',cancelButtonColor:'#6c757d',confirmButtonText:'Ya, Nonaktifkan',cancelButtonText:'Batal',reverseButtons:true}).then((r)=>{if(r.isConfirmed){$wire.delete({{ $item->id }})}})}" 
                ...
                <div class="px-6 py-5 sm:px-8 sm:py-6 bg-gray-50/50 border-t border-gray-100 pagination-custom">
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-5">
                        <div class="text-[11px] font-bold text-[#878a99] tracking-tight text-center sm:text-left">
                            ...
                            <span class="text-[#405189] font-black">{{ $this->menus->firstItem() }} - {{ $this->menus->lastItem() }}</span> 
                            dari <span class="text-[#405189] font-black">{{ number_format($this->menus->total()) }}</span> 
                            <span class="hidden sm:inline">menu terdaftar</span>
                ...
                <div x-show="showModal" x-transition.scale.95 
                    ...
                    <div class="px-5 py-4 sm:px-8 sm:py-6 bg-gradient-to-r from-gray-50 to-white border-b border-gray-100 flex items-center justify-between">
                        <div class="flex items-center gap-2 sm:gap-3">
                            ...
                            <div>
                                <h5 class="text-sm sm:text-base font-black text-[#2c3e50] tracking-tight">{{ $isEdit ? 'Update Data Menu' : 'Menu Baru' }}</h5>
                                <p class="text-[9px] sm:text-[10px] text-gray-400 font-bold uppercase tracking-widest hidden sm:block">Lengkapi informasi menu di bawah</p>
                            </div>
                        </div>
                    ...
                    <div class="px-5 py-6 sm:px-8 sm:py-8 max-h-[70vh] overflow-y-auto scrollbar-hide">
                        <form wire:submit.prevent="save" class="space-y-5 sm:space-y-6">
                            <div class="space-y-1.5">
                                <label class="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Nama Menu <span class="text-rose-500">*</span></label>
                                <input type="text" wire:model="menu_name" x-ref="firstInput" 
                                       class="w-full bg-gray-50 border border-gray-100 rounded-xl sm:rounded-2xl py-2.5 sm:py-3 px-4 text-sm font-bold text-[#2c3e50] focus:bg-white focus:ring-4 focus:ring-indigo-100 focus:border-[#405189] transition-all outline-none @error('menu_name') border-rose-300 bg-rose-50/30 @enderror" 
                                       placeholder="Contoh: Dashboard">
                                @error('menu_name') <span class="text-[10px] text-rose-500 font-bold px-1">{{ $message }}</span> @enderror
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
                                <div class="space-y-1.5">
                                    <label class="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Link/URL</label>
                                    <input type="text" wire:model="menu_link" 
                                ...
                                <div class="space-y-1.5">
                                    <label class="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Pilih Icon</label>
                                    <x-custom-dropdown 
                                        model="menu_icon" 
                                        :options="$iconList"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
                                <div class="space-y-1.5">
                                    <label class="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Parent Menu</label>
                                    <x-custom-dropdown 
                                        model="parent_id" 
                                        :options="$parentList"
                                ...
                                <div class="space-y-1.5">
                                    <label class="text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Urutan</label>
                            ...
                            <div class="flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-xl sm:rounded-2xl border border-dashed border-gray-200">
                                <div class="flex items-center gap-2 sm:gap-3">
                                    <div class="flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-lg {{ $is_active ? 'bg-emerald-100 text-emerald-600 uppercase' : 'bg-rose-100 text-rose-600' }} shadow-sm">
                                        <i class="ri-{{ $is_active ? 'check-line' : 'close-line' }} text-base sm:text-lg font-bold"></i>
                                    </div>
                                    <div>
                                        <p class="text-[8px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none">Status Menu</p>
                                        <p class="text-[11px] sm:text-xs font-black {{ $is_active ? 'text-emerald-600' : 'text-rose-600' }} mt-1">{{ $is_active ? 'AKTIF' : 'TIDAK-AKTIF' }}</p>
                                    </div>
                                </div>
                                <label class="relative inline-flex items-center cursor-pointer scale-90 sm:scale-100">
                                    <input type="checkbox" class="sr-only peer" {{ $is_active ? 'checked' : '' }} @click="$wire.set('is_active', {{ $is_active ? 'false' : 'true' }})">
