        ...
        <table>
            ...
            <tr>
                ...
                <td>: {{ $pasien->tanggal_lahir ? date('d/m/Y', strtotime($pasien->tanggal_lahir)) : '-' }}
                    ({{ $pasien->tanggal_lahir ? floor((time() - strtotime($pasien->tanggal_lahir)) / 31556926) : '-' }}
                    thn)</td>
            </tr>
        </table>
    </div>
    ...
    @foreach($historyData as $idx => $data)
        <div class="history-item">
            <div class="history-date">
                <span>Kunjungan #{{ count($historyData) - $idx }} -
                    {{ date('d F Y', strtotime($data['pendaftaran']->created_at)) }}</span>
            ...
            <div class="content-section">
                <table class="grid">
                    <tr>
                        ...
                        <td width="35%">{{ $data['pendaftaran']->dokter ? $data['pendaftaran']->dokter->nama_dokter : '-' }}
                        </td>
                        <td class="bg-gray">Pemeriksaan Awal</td>
                        <td>
                            <span style="color:#666; font-size:7pt;">TD:</span>
                            {{ $data['clinical']['pemeriksaan_awal']['td'] ?: '-' }} |
                            <span style="color:#666; font-size:7pt;">N:</span>
                            {{ $data['clinical']['pemeriksaan_awal']['nadi'] ?: '-' }} |
                            <span style="color:#666; font-size:7pt;">S:</span>
                            {{ $data['clinical']['pemeriksaan_awal']['suhu'] ?: '-' }} |
                            <span style="color:#666; font-size:7pt;">BB:</span>
                            {{ $data['clinical']['pemeriksaan_awal']['bb'] ?: '-' }}
                        </td>
                    </tr>
                </table>
                ...
                <div class="soap-box">
                    <div class="soap-line"><span class="soap-label">S:</span>
                        {{ $data['clinical']['soap']->subjective ?? '-' }}</div>
                    <div class="soap-line"><span class="soap-label">O:</span>
                        {{ $data['clinical']['soap']->objective ?? '-' }}</div>
                    <div class="soap-line"><span class="soap-label">A:</span>
                        {{ $data['clinical']['soap']->assessment ?? '-' }}</div>
                    <div class="soap-line"><span class="soap-label">P:</span>
                        {{ $data['clinical']['soap']->planning ?? '-' }}</div>
                </div>
                        ...
                        <tr>
                            <td class="diagnosis-cell">
                                @forelse($data['clinical']['diagnoses'] as $diag)
                                    <div style="margin-bottom:2px;">• <strong>{{ $diag->kode_diagnosa }}</strong> -
                                        {{ $diag->nama_diagnosa }}</div>
                                @empty
                                    -
                                @endforelse

                                <div style="margin-top:8px; border-top:1px dashed #eee; padding-top:5px;">
                                    <div style="font-size:7pt; color:#405189; font-weight:bold; margin-bottom:3px;">GIGI
                                        DIPERIKSA:</div>
                                    @forelse($data['clinical']['odontogram_visit'] as $gv)
                                        <div style="font-size:7.5pt; margin-bottom:1px;">• Gigi
                                            <strong>{{ $gv->nomor_gigi }}</strong> ({{ $gv->bagian }}):
                                            {{ $gv->nama_kategori ?: 'Pemeriksaan' }}</div>
                                    @empty
                            ...
                            <td>
                                @forelse($data['clinical']['obat'] as $o)
                                    <div style="margin-bottom:2px;">• <strong>{{ $o->nama_obat }}</strong> ({{ $o->dosis }})
                                    </div>
                                @empty
                                    -
                                @endforelse
        ...
        <table class="odontogram-layout">
            <tr>
                ...
                <td class="odontogram-legend">
                    <div
                        style="font-size: 8pt; font-weight: bold; color: #405189; margin-bottom: 8px; border-bottom: 1px solid #405189;">
                        LEGENDA</div>
            ...
            <table class="ohis-table">
                <tr>
                    ...
                    <td class="ohis-cell">
                        ...
                        <span class="ohis-item-val" style="color:#000000 !important;">
                            {{ strtoupper($latestOhis->kategori) }}
                        </span>
                    </td>
                </tr>
            </table>
