...
<html>
<head>
    <title>Data Menu - SIGI Dental EMR</title>
...
<body>
    <div class="header">
        ...
        <p>Laporan Data Menu</p>
        <p>Dicetak pada: {{ date('d F Y H:i') }}</p>
    </div>
    <table>
        <thead><tr><th width="5%">No</th><th>Nama Menu</th><th>Link</th><th>Icon</th><th>Parent ID</th><th>Urutan</th><th>Status</th></tr></thead>
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $item->order_no }}</td>
                <td><span class="status-badge {{ $item->is_active ? 'status-aktif' : 'status-tidak-aktif' }}">{{ $item->is_active ? 'Aktif' : 'Tidak Aktif' }}</span></td>
            </tr>
            @endforeach
        </tbody>
    </table>
