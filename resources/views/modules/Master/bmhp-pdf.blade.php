...
<html>
<head>
    <title>Data BMHP - SIGI Dental EMR</title>
...
<body>
    <div class="header">
        ...
        <p>Laporan Data BMHP</p>
        <p>Dicetak pada: {{ date('d F Y H:i') }}</p>
    </div>
    <table>
        <thead><tr><th width="5%">No</th><th>Kode</th><th>Nama BMHP</th><th>Satuan</th><th>Stok</th><th>Harga Satuan</th><th>Status</th></tr></thead>
        <tbody>
            @foreach($dataList as $index => $item)
            <tr>
                <td>{{ $index + 1 }}</td>
                <td>{{ $item->kode_bmhp }}</td>
